...
class AutomationResource(BaseResource):
    ...
    def approve(self, pk, data):
        # type: (str, dict) -> str
        return self.client.post(path=pk + '/approve/', json=data)[0]

    @function_log
    def inquire(self, pk):
        # type: (str) -> str
        return self.client.post(path=pk + '/inquire/', json={})[0]

    @function_log
    def fail(self, pk, reason):
        # type: (str, str) -> str
        return self.client.post(path=pk + '/fail/', json={'reason': reason})[0]
